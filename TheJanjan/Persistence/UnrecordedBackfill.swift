...
///
/// **복약률의 분모를 만들려는 것이 아니다**(사용자 결정 2026-09-21).
/// 처음에는 그 목적이었다 - 앱을 덜 여는 사람일수록 숫자가 더 좋게 나오는
/// 역설을 막으려 했다. 그런데 계획은 저장하지 않고 지금의 요일로 매번 다시
/// 그리는 구조라, 요일을 넓히기만 해도 여기서 채운 줄들이 지난 4주를
/// "빠트림" 으로 덮어 100% 가 42% 로 내려갔다. 한 번도 안 빠트린 사람의
/// 숫자였다. 지금은 미기록이 비율의 분모에 들어가지 않고, 대신 비율 옆에
/// 답한 날 수가 함께 나간다.
///
/// 지키는 것 셋.
///
